...
void single_buffer() {
	...
	/* Create bitmap used to simulate double buffering */
	page = create_bitmap(SCREEN_W, SCREEN_H);
	__ASSERT(page, ERROR_MEM_ALLOC);
	...
	while(i == 0 && !keypressed()) {
		...
		while(repeat++ < 8 && i == 0 && !keypressed()) {
			acquire_bitmap(page);
			stretch_blit(background, page, 0, 0, background->w, background->h, 0, 0, SCREEN_W, SCREEN_H); // background
			if(repeat%2 == 1)
				textout_centre_ex(page, font_msg, WIIMOTE_CONNECTION_MSG, SCREEN_W/2, SCREEN_H/2, makecol(0, 0, 0), -1); // text prompt
			release_bitmap(page);
			blit(page, screen, 0, 0, 0, 0, SCREEN_W, SCREEN_H); // release frame on the screen
	...
	for(repeat=0;repeat<2;repeat++) {
		...
		while(!keypressed()) {
			...
			acquire_bitmap(page);
			/* background */
			stretch_blit(background, page, 0, 0, background->w, background->h, 0, 0, SCREEN_W, SCREEN_H);
			...
			switch(repeat) {
				case 0:
					textout_centre_ex(page, font_msg, WIIMOTE_CAL_ASX, SCREEN_W/2, SCREEN_H/2, makecol(0, 0, 0), -1); // text prompt
					rect(page, dot[0].x, dot[0].y, SCREEN_W+1, SCREEN_H+1, makecol(0, 0, 0));
					break;
				case 1:
					textout_centre_ex(page, font_msg, WIIMOTE_CAL_BDX, SCREEN_W/2, SCREEN_H/2, makecol(0, 0, 0), -1); // text prompt
					
					rect(page, dot[1].x, dot[1].y, SCREEN_W+1, SCREEN_H+1, makecol(0, 0, 0));
					rect(page, -1, -1, dot[0].x, dot[0].y, makecol(0, 0, 0));
					break;
			}
			circlefill(page, dot[0].x, dot[0].y, radius, makecol(0, 0, 0));
			
			release_bitmap(page);
			/* Release actual frame on the screen*/
	...
	while (TRUE) {
		...
		for(i=0;i<NUM_BARS;i++) {
			...
			/* Draw bar */
			rectfill(page, bar[i].min.x, bar[i].min.y, bar[i].max.x, bar[i].max.y, makecol(bar[i].color, bar[i].color, bar[i].color));
			/* Print bar's associated note */
			textout_centre_ex(page, font_notes, bar[i].note, (bar[i].min.x + half_bar_size), text_bar_height, makecol(0, 0, 0), -1);
		}
		...
		for(i=0;i<MAX_IR_DOTS;i++) {
			if (wiimote->ir.dot[i].visible) { // if ir source is visible
				...
				if (dot[i].y > bar[0].min.y-radius) {
					...
					circlefill(page, dot[i].x, bar[0].min.y-radius, radius, makecol(0, 0, 0));
				}
				else	circlefill(page, dot[i].x, dot[i].y, radius, makecol(0, 0, 0));
